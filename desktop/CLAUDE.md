...
**カード全体のツールチップ（`title`）**: 各カード（＝5時間 / 7日の1ウィンドウ）の `.usage-bar` に `使用量｜リセット日付｜経過%｜残り` を改行区切りでまとめた `title` を付ける（`tooltip` 変数、全サイズモード共通）。経過%は独立表示を持たずこのツールチップに集約。**weekly_scoped（Scoped）副バーは同じカード内に入るためこの主ウィンドウのツールチップを共有する＝ツールチップに Scoped の数値は出さない**。個別セル・マーカーに `title` は付けない（カードの `title` が全域で出るように）。
